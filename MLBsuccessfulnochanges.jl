...
function one_lineup_Type_4(hitters, pitchers, lineups, num_overlap, num_hitters, num_pitchers, catchers, firstbasemen, secondbasemen, thirdbasemen, shortstops, outfielders, num_teams, hitters_teams, pitcher_opponents, team_lines, num_lines, P1_info)
    ...
    # @addConstraint(m, sum{sum{defenders[i]*P1_info[i,j]*hitters_lineup[i], i=1:num_hitters}, j=1:num_teams} ==  sum{defenders[i]*hitters_lineup[i], i=1:num_hitters})
...
function create_lineups(num_lineups, num_overlap, path_hitters, path_pitchers, formulation, path_to_output)
    ...
    PP_info = zeros(Int, num_hitters)
    for num=1:size(hitters)[1]
        if hitters[:Team][num]==teams[1]
            if hitters[:Power_Play][num] == "1"
                PP_info[num] = 1
            end
        end
    end
    ...
    the_lineup= formulation(hitters, pitchers, hcat(zeros(Int, num_hitters + num_pitchers), zeros(Int, num_hitters + num_pitchers)), num_overlap, num_hitters, num_pitchers, catchers, firstbasemen, secondbasemen, thirdbasemen, shortstops, outfielders, num_teams, hitters_teams, pitcher_opponents, team_lines, num_lines, P1_info)
    the_lineup2 = formulation(hitters, pitchers, hcat(the_lineup, zeros(Int, num_hitters + num_pitchers)), num_overlap, num_hitters, num_pitchers, catchers, firstbasemen, secondbasemen, thirdbasemen, shortstops, outfielders, num_teams, hitters_teams, pitcher_opponents, team_lines, num_lines, P1_info)
    tracer = hcat(the_lineup, the_lineup2)
    for i=1:(num_lineups-2)
        try
            thelineup=formulation(hitters, pitchers, tracer, num_overlap, num_hitters, num_pitchers, catchers, firstbasemen, secondbasemen, thirdbasemen, shortstops, outfielders, num_teams, hitters_teams, pitcher_opponents, team_lines, num_lines, P1_info)
